# ShockForce
### OVERVIEW
This program models the static forces of a motorcycle front suspension.  This is intended as a tool to both educate and inform riders how common, easily changed paramaters affect a motorcycle suspension.  This is achieved through rendering a series of graphs where a single paramater is changed in linear space and assembling these into an animation.  The user interface was created using Qt (pyQt5) and the data visualization with Matplotlib.  The data for these animations is generated by a "homespun" class using a methodology described below.
